...
from xml.etree.ElementTree import (  # , dump, fromstring, register_namespace, ElementTree, tostring
    fromstring,
    parse,
)
...
class EntrySet(pyproteins.container.customCollection.EntrySet):
    def __init__(self, **kwargs):
        self.ns = "{https://uniprot.org/uniprot}"
        self.old_ns = "{http://uniprot.org/uniprot}"
        ...
        if "streamXML" or "collectionXML" in kwargs:
            ...
            if not self.etree_root.findall(f"{self.ns}entry"):
                self.ns = self.old_ns
                if not self.etree_root.findall(f"{self.ns}entry"):
                    print("Input collection seems empty!")

        super().__init__(
            collectionPath=cachePath,
            constructor=Entry,
            typeCheck=isValidID,
            indexer=strip,
        )
        if self.isXMLCollection:
            print(f"Succesfully parsed a {len(self)} entries xml collection")
        ...
        if self.isXMLCollection:
            for entry in self.etree_root.findall(f"{self.ns}entry"):
                for acc in entry.findall(f"{self.ns}accession"):
                    if (
                        acc.text == uniprotID
                    ):  # entry is the node matching provided UNIPROT accessor
